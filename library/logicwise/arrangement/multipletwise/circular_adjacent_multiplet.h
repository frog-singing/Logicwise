...
namespace logicwise::detail
{
	...
	struct circular_adjacent_multiplet_impl : multipletwise_arrangement_tag
	{
		using type = circular_adjacent_multiplet_impl;
	...
	template<> struct circular_adjacent_multiplet_impl<0> { using type = multipletwise_void; };
	template<> struct circular_adjacent_multiplet_impl<1> { using type = multipletwise_element; };
	template<> struct circular_adjacent_multiplet_impl<2> { using type = circular_adjacent_pair; };

	template<std::integral auto Arity> requires (Arity >= 0)
	using circular_adjacent_multiplet = typename circular_adjacent_multiplet_impl<static_cast<std::size_t>(Arity)>::type;
