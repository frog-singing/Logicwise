...
namespace logicwise::detail
{
	...
	//单向线性相邻对
	struct linear_adjacent_pair : pairwise_arrangement_tag
	{
		...
		static constexpr std::size_t index_count(extent_type extent)
		{
			...
			return extent_i < 2 ? 0 : extent_i - 1;
		}
		...
		struct forward_index_traverser
		{
			...
			{
				assert(!done() && index_i >= 0 && index_j >= 0 && "[logicwise] Accessing illegal index.");
		...
		struct reverse_index_traverser
		{
			...
			explicit constexpr reverse_index_traverser(extent_type extent) noexcept
				: index_j{ safe_integer_cast<index_integer_type>(extent.i()) - 1 }
				, index_i{ index_j - 1 }
			{}
